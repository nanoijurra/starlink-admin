...
export function mensajePorCargo(persona, cargo, mes, aliasBancario = '') {
  if (persona.estado === 'SUSPENDIDO_MORA') {
    return `Hola ${persona.nombre}. Registrás mora en el servicio Starlink ACC Córdoba. Para regularizar el acceso, comunicate con la administración.`;
  }
  ...
  if (cargo?.concepto_equipo === 'COMPRA_INICIAL' || cargo?.compra_inicial_aplicada > 0) {
    return `Hola ${persona.nombre}. Starlink ACC Córdoba.
Mes: ${mes}
...
Mes: ${mes}
Importe a pagar: ${monto}
Concepto: regularización proporcional de compra inicial + abono mensual.
La regularización se usa dentro de la cuota mensual para armonizar los aportes. Una vez equilibrado el costo inicial, pasás a cuota normal.${alias}`;
  }
